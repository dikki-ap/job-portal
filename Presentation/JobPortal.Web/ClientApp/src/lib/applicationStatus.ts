...
export type DerivedStatus = 'Pending' | 'InReview' | 'Accepted' | 'Rejected';
...
export function deriveStatus(app: Pick<ApplicationDto, 'status' | 'steps'>): DerivedStatus {
  ...
  // DB status is 'Pending' — refine to InReview if at least one step has been passed
  const anyPassed = app.steps.some((s) => s.status === 'Passed');
  return anyPassed ? 'InReview' : 'Pending';
}
