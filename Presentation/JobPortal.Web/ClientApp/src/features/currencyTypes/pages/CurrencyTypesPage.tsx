...
import { CurrencyTypeForm } from '../components/CurrencyTypeForm';
import { CurrencyTypesTable } from '../components/CurrencyTypesTable';
import { useGetCurrencyTypesQuery } from '../api/currencyTypesApi';
import { usePagination } from '../../../hooks/usePagination';
import { useToast } from '../../../hooks/useToast';
import type { CurrencyTypeDto } from '../../../types/api';

export function CurrencyTypesPage() {
  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<CurrencyTypeDto | null>(null);
  const [search, setSearch] = useState('');
  const { toasts, addToast, dismissToast } = useToast();

  const { data: currencyTypes = [], isLoading, isError } = useGetCurrencyTypesQuery();

  const filtered = useMemo(
    () => currencyTypes.filter(
      (c) => c.name.toLowerCase().includes(search.toLowerCase()) ||
             c.prefix.toLowerCase().includes(search.toLowerCase())
    ),
    [currencyTypes, search]
  );

  const { paginated, currentPage, totalPages, totalItems, pageSize, from, to, goToPage, setPageSize } =
    usePagination(filtered);

  const handleEdit = (item: CurrencyTypeDto) => { setEditing(item); setFormOpen(true); };
  const handleClose = () => { setFormOpen(false); setEditing(null); };

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-gray-900">Currency Type Management</h1>
        <p className="text-sm text-gray-500">Manage currency types available in the job portal.</p>
      </div>
      ...
      <CurrencyTypeForm open={formOpen} onClose={handleClose} editing={editing} onSuccess={(msg) => addToast(msg, 'success')} onError={(msg) => addToast(msg, 'error')} />
