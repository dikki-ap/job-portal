...
export const documentTypesApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createDocumentType: builder.mutation<DocumentTypeDto, { name: string; maxFileSizeMb: number; isDefaultRequired: boolean; mimeTypes: string[] }>({
      query: (body) => ({ url: '/document-types', method: 'POST', body }),
      invalidatesTags: ['DocumentType'],
    }),
    updateDocumentType: builder.mutation<DocumentTypeDto, { id: number; name: string; maxFileSizeMb: number; isDefaultRequired: boolean; mimeTypes: string[] }>({
