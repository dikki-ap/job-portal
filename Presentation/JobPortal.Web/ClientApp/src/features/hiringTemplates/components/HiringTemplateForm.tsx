...
import { useCreateHiringTemplateMutation, useUpdateHiringTemplateMutation } from '../api/hiringTemplatesApi';
...
export function HiringTemplateForm({ open, onClose, editing, onSuccess, onError }: HiringTemplateFormProps) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [steps, setSteps] = useState<StepItem[]>([{ tempId: crypto.randomUUID(), name: '', isRequired: true }]);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const [createTemplate, { isLoading: isCreating }] = useCreateHiringTemplateMutation();
  const [updateTemplate, { isLoading: isUpdating }] = useUpdateHiringTemplateMutation();
    ...
    if (open) {
      ...
      setSteps(
        editing?.steps.length
          ? editing.steps.map((s) => ({ tempId: crypto.randomUUID(), name: s.name, isRequired: s.isRequired }))
          : [{ tempId: crypto.randomUUID(), name: '', isRequired: true }]
      );
      setErrors({});
    }
  }, [open, editing]);
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="ht-desc" className="text-sm font-medium text-gray-700">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">Hiring Steps</span>
            ...
            <div key={step.tempId} className="flex items-center gap-2">
              ...
              <label className="flex items-center gap-1.5 text-xs text-gray-600 shrink-0">
                <input
                  ...
                  onChange={(e) => updateStep(step.tempId, 'isRequired', e.target.checked)}
                  disabled={isLoading}
