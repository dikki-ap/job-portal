...
interface EducationMajorsTableProps {
  educationMajors: EducationMajorDto[];
  onEdit: (item: EducationMajorDto) => void;
...
export function EducationMajorsTable({ educationMajors, onEdit, onSuccess, onError }: EducationMajorsTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteEducationMajor(confirmItem.id).unwrap();
      onSuccess(`"${confirmItem.name}" has been deleted.`);
    } catch {
      onError("Failed to delete education major. It may be in use.");
    } finally {
      setDeletingId(null);
      setConfirmItem(null);
    }
  ...
  if (educationMajors.length === 0) {
    return <div className="flex flex-col items-center justify-center py-16 text-gray-400"><p className="text-sm">No education majors found.</p></div>;
  }
  ...
  return (
    <>
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {educationMajors.map((em, idx) => (
              <tr key={em.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 text-gray-400 font-mono text-xs">{idx + 1}</td>
                <td className="px-6 py-4 font-medium text-gray-900">{em.name}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{em.createdByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(em.createdAt)}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{em.updatedByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(em.updatedAt)}</td>
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon" onClick={() => onEdit(em)} title="Edit"><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="text-red-500 hover:bg-red-50 hover:text-red-600" onClick={() => setConfirmItem(em)} loading={deletingId === em.id} title="Delete"><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Modal open={!!confirmItem} onClose={() => setConfirmItem(null)} title="Delete Education Major">
