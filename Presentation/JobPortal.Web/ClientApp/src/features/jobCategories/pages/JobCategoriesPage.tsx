...
import { useGetJobCategoriesQuery } from '../api/jobCategoriesApi';
...
export function JobCategoriesPage() {
  ...
  const { data: jobCategories = [], isLoading, isError } = useGetJobCategoriesQuery();

  const filtered = useMemo(
    () => jobCategories.filter((j) => j.name.toLowerCase().includes(search.toLowerCase())),
    [jobCategories, search]
  );

  const { paginated, currentPage, totalPages, totalItems, pageSize, from, to, goToPage, setPageSize } =
    usePagination(filtered);

  const handleEdit = (item: JobCategoryDto) => {
    setEditing(item);
    setFormOpen(true);
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-gray-900">Job Category Management</h1>
        <p className="text-sm text-gray-500">Manage job categories available in the job portal.</p>
      </div>
