...
interface EmploymentTypesTableProps {
  ...
  onEdit: (item: EmploymentTypeDto) => void;
...
export function EmploymentTypesTable({ employmentTypes, onEdit, onSuccess, onError }: EmploymentTypesTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteEmploymentType(confirmItem.id).unwrap();
      onSuccess(`"${confirmItem.name}" has been deleted.`);
    } catch {
      onError("Failed to delete employment type. It may be in use.");
    } finally {
      setDeletingId(null);
      setConfirmItem(null);
    }
  ...
  if (employmentTypes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-gray-400">
        <p className="text-sm">No employment types found.</p>
      </div>
    );
  }
  ...
  return (
    <>
          ...
          <tbody className="divide-y divide-gray-100">
            {employmentTypes.map((et, idx) => (
              <tr key={et.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 font-medium text-gray-900">{et.name}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{et.createdByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(et.createdAt)}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{et.updatedByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(et.updatedAt)}</td>
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon" onClick={() => onEdit(et)} title="Edit">
      ...
      <Modal open={!!confirmItem} onClose={() => setConfirmItem(null)} title="Delete Employment Type">
