...
export function JobLevelsTable({ jobLevels, onEdit, onSuccess, onError }: JobLevelsTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteJobLevel(confirmItem.id).unwrap();
      onSuccess(`"${confirmItem.name}" has been deleted.`);
    } catch {
      onError("Failed to delete job level. It may be in use.");
    } finally {
      setDeletingId(null);
      setConfirmItem(null);
    }
  ...
  if (jobLevels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-gray-400">
        <p className="text-sm">No job levels found.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {jobLevels.map((jl, idx) => (
              <tr key={jl.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 text-gray-400 font-mono text-xs">{idx + 1}</td>
                <td className="px-6 py-4 font-medium text-gray-900">{jl.name}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{jl.createdByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(jl.createdAt)}</td>
                <td className="px-6 py-4 text-gray-500 hidden lg:table-cell">{jl.updatedByName ?? "—"}</td>
                <td className="px-6 py-4 text-gray-500 hidden md:table-cell">{formatDateTime(jl.updatedAt)}</td>
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon" onClick={() => onEdit(jl)} title="Edit">
      ...
      <Modal open={!!confirmItem} onClose={() => setConfirmItem(null)} title="Delete Job Level">
