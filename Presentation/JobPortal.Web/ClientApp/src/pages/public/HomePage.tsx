...
function HeroSection() {
  ...
  return (
    <section
      ...
      style={{
        background: 'linear-gradient(135deg, #004181 0%, #0066cc 50%, #0080ff 100%)',
      }}
    >
      {/* Decorative circles */}
      ...
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        <span className="inline-block rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-xs font-medium text-blue-100 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-[#004181] hover:bg-blue-50 transition-colors shadow-lg"
          >
    ...
    <section id="services" className="py-20 bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="text-sm font-semibold text-[#004181] uppercase tracking-wider">What We Do</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => (
            <div
              key={service.id}
              className="group rounded-2xl bg-white p-6 shadow-sm border border-gray-100 hover:border-[#004181]/20 hover:shadow-md transition-all"
            >
              <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-[#004181] group-hover:bg-[#004181] group-hover:text-white transition-colors">
...
function ProjectsSection() {
  ...
  return (
    <section id="projects" className="py-20 bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="text-sm font-semibold text-[#004181] uppercase tracking-wider">Our Work</span>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="flex flex-col gap-6">
            <div>
              <span className="text-sm font-semibold text-[#004181] uppercase tracking-wider">Who We Are</span>
              <h2 className="mt-2 text-3xl font-bold text-gray-900">About {company.name}</h2>
            </div>
            <p className="text-gray-600 leading-relaxed">
              Founded in {company.foundedYear}, {company.name} has grown from a boutique IT consulting firm into
            ...
            <div className="flex flex-col gap-4">
              {values.map((value) => (
                <div key={value.title} className="flex gap-3">
                  <CheckCircle2 className="h-5 w-5 text-[#004181] shrink-0 mt-0.5" />
                  <div>
            ...
            {stats.map((stat, idx) => (
              <div
                ...
                className={`rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-1 ${
                  idx % 2 === 0 ? 'bg-[#004181] text-white' : 'bg-white border border-gray-100 shadow-sm'
                }`}
              >
                <span className={`text-4xl font-bold ${idx % 2 === 0 ? 'text-white' : 'text-[#004181]'}`}>
                  {stat.value}
                </span>
                <span className={`text-xs font-medium ${idx % 2 === 0 ? 'text-blue-200' : 'text-gray-500'}`}>
                  {stat.label}
                </span>
              </div>
            ))}
...
function JobCard({ job, onView }: { job: JobPostDto; onView: () => void }) {
  return (
    <div className="group flex items-center gap-4 rounded-xl border border-gray-100 bg-white p-4 hover:border-[#004181]/20 hover:shadow-sm transition-all cursor-pointer" onClick={onView}>
      ...
      <ChevronRight className="h-4 w-4 text-gray-300 group-hover:text-[#004181] shrink-0 transition-colors" />
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-12">
          <div className="lg:w-80 shrink-0 flex flex-col gap-4">
            <span className="text-sm font-semibold text-[#004181] uppercase tracking-wider">Join the Team</span>
            ...
            <button
              onClick={() => navigate('/careers')}
              className="mt-2 inline-flex items-center gap-2 rounded-xl bg-[#004181] px-5 py-3 text-sm font-semibold text-white hover:bg-[#003166] transition-colors w-fit"
            >
...
function ContactSection() {
  return (
    <section id="contact" className="py-20 bg-[#004181]">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-[#004181] hover:bg-blue-50 transition-colors"
          >
        ...
        <p className="text-blue-200 text-sm">{company.address}</p>
      </div>
    </section>
  );
}
