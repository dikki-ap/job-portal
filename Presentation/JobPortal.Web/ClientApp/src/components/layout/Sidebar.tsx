  ...
  if (item.children) {
    return (
      <div>
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-white/80 hover:bg-white/10 transition-colors"
        >
          {item.icon}
    ...
    <>
      {sections.map((section, si) => (
        <div key={section.label} className={cn('flex flex-col gap-0.5', si > 0 && 'mt-4')}>
          <p className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-widest text-white/50">
            {section.label}
          </p>
...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-30 flex w-64 flex-col bg-[var(--primary)] transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0',
      ...
      >
        <div className="flex h-16 items-center gap-3 border-b border-white/20 px-5">
          ...
          <span className="text-lg font-bold text-white flex-1">{companyName}</span>
          <a
            href="/"
            target="_blank"
